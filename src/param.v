...
`ifdef TESTMODE
    // Асинхронное FIFO глубиной 8 элементов для безопасной передачи шины (поведенческая симуляция)
    param_async_fifo #(
        .DATA_WIDTH (64),
        .ADDR_WIDTH (3)
    ) u_async_fifo (
...
`else
    // Использование стандартного аппаратного макроса Quartus dcfifo
    wire aclr = !src_rst_n;
