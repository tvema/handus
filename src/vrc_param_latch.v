...
// Подключение файла с дефолтными параметрами
`include "def_param.h"
...
module vrc_param_latch (
    ...
    always @(posedge dac_clk or negedge dac_rst_n) begin
        if (!dac_rst_n) begin
            hold_vrc_type    <= `INIT_VRC_TYPE;
            hold_dac_div     <= `INIT_VRC_DAC_DIV;
            hold_start_delay <= `INIT_VRC_START_DELAY;
            hold_init_gain   <= `INIT_VRC_INIT_GAIN;
            hold_rate_1      <= `INIT_VRC_RATE_1;
            hold_duration_1  <= `INIT_VRC_DURATION_1;
            hold_rate_2      <= `INIT_VRC_RATE_2;
            hold_duration_2  <= `INIT_VRC_DURATION_2;
            hold_dac_min     <= `INIT_VRC_DAC_MIN;
            hold_dac_max     <= `INIT_VRC_DAC_MAX;
    ...
    always @(posedge dac_clk or negedge dac_rst_n) begin
        if (!dac_rst_n) begin
            o_vrc_type    <= `INIT_VRC_TYPE;
            o_dac_div     <= `INIT_VRC_DAC_DIV;
            o_start_delay <= `INIT_VRC_START_DELAY;
            o_init_gain   <= `INIT_VRC_INIT_GAIN;
            o_rate_1      <= `INIT_VRC_RATE_1;
            o_duration_1  <= `INIT_VRC_DURATION_1;
            o_rate_2      <= `INIT_VRC_RATE_2;
            o_duration_2  <= `INIT_VRC_DURATION_2;
            o_dac_min     <= `INIT_VRC_DAC_MIN;
            o_dac_max     <= `INIT_VRC_DAC_MAX;
